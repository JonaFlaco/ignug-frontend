...
import { CreateCatalogueTypeDto, CatalogueTypeModel, UpdateCatalogueTypeDto } from '@models/uic';
...
export class CatalogueTypesHttpService {
  ...
  remove(id: string): Observable<CatalogueTypeModel> {
    const url = `${this.API_URL}/${id}`;
  ...
  removeAll(catalogueType: CatalogueTypeModel[]): Observable<CatalogueTypeModel[]> {
    const url = `${this.API_URL}/remove-all`;

    this.coreService.showLoad();
    return this.httpClient.patch<ServerResponse>(url, catalogueType).pipe(
      map((response) => {
        this.coreService.hideLoad();
        this.messageService.success(response).then();
        return response.data;
