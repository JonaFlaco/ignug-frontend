...
import {CreateEvaluationDto, UpdateEvaluationDto, EvaluationModel } from '@models/uic';
...
export class EvaluationHttpService {
  API_URL = `${environment.API_URL}/evaluations`;
  ...
  evaluation(type: CatalogueTypeEnum): Observable<EvaluationModel[]> {
  ...
  removeAll(evaluation: EvaluationModel[]): Observable<EvaluationModel[]> {
    const url = `${this.API_URL}/remove-all`;

    this.coreService.showLoad();
    return this.httpClient.patch<ServerResponse>(url, evaluation).pipe(
      map((response) => {
        this.coreService.hideLoad();
        this.messageService.success(response).then();
        return response.data;
